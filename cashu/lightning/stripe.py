...
class StripeWallet(LightningBackend):
    ...
    checking_ids to the `cashu:paid_invoices` Redis channel when Stripe
    webhooks confirm payment. This backend subscribes to that channel
    and yields checking_ids for Nutshell's invoice_callback_dispatcher.
    ...
    def __init__(self, unit: Unit, **kwargs):
        ...
        Raises:
            ...
            ValueError: If MINT_REDIS_URL is not configured.
        """
    ...
    async def create_invoice(
        ...
        and payment_request. The Go API creates the actual Stripe checkout
        session separately and maps the quote to the Stripe session.
        ...
        Returns:
            InvoiceResponse with a UUID as both checking_id and payment_request.
        """
        checking_id = str(uuid.uuid4())
        return InvoiceResponse(
            ok=True,
            checking_id=checking_id,
            payment_request=checking_id,
        )
    ...
    async def paid_invoices_stream(self) -> AsyncGenerator[str, None]:
        ...
        The Go API publishes checking_ids to the `cashu:paid_invoices`
        Redis channel when Stripe webhooks confirm payment. This method
        subscribes to that channel and yields checking_ids as they arrive.
